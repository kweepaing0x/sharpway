  ...
  try {
    ...
    switch (req.method) {
      ...
      case "DELETE": {
        ...
        // Also check if any products are using this category
        ...
        if (productData && productData.length > 0) {
          return new Response(
            JSON.stringify({ error: 'Cannot delete category that is used by products' }),
            { 
              status: 400, 
              headers: { ...corsHeaders, "Content-Type": "application/json" }
            }
    ...
    console.error("Error in product-category-manager:", error);
